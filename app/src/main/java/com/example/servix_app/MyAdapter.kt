...
class MyAdapter(var announements: List<Announcement>) : RecyclerView.Adapter<MyAdapter.MyViewHolder>() {
    ...
    override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
        ...
        Picasso.get().load(announements[position].imagesURL).into(images)
        title.text = announements[position].title
        state.text = announements[position].state
        location.text = announements[position].state
        date.text = announements[position].date.toString()
    }
}